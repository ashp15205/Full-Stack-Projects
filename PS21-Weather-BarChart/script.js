...
const cityA = {
  name: "Mumbai",
  temps: [30, 32, 31, 33, 29, 28, 30],
};
const cityB = {
  name: "Delhi",
  temps: [22, 25, 24, 27, 26, 23, 21],
};

// Simulate Promise.all (key ES6 concept)
const mockFetch = (city) => Promise.resolve(city);
Promise.all([mockFetch(cityA), mockFetch(cityB)]).then(
  ([a, b]) => {
    ...
    // Stats
    const avgA = (
      a.temps.reduce((s, v) => s + v, 0) / a.temps.length
    ).toFixed(1);
    const avgB = (
      b.temps.reduce((s, v) => s + v, 0) / b.temps.length
    ).toFixed(1);
